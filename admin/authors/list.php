...
<body>
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.delete-author-btn', function() {
            ...
            if (confirm(
                "Are you absolutely sure you want to permanently delete this author profile?")) {
                $.ajax({
                    url: 'ajax/delete.php',
                    type: 'POST',
                    data: {
                    ...
                    if (authors.length > 0) {
                        $.each(authors, function(index, row) {
                            ...
                            if (row.image && row.image.trim() !== "") {
                                ...
                                imgHtml = '<img src="' + finalImgSrc +
                                    '" width="50" height="50" style="object-fit: cover;" class="rounded-circle" alt="Author">';
                            } else {
                                imgHtml =
                                    '<div class="bg-secondary text-white rounded-circle d-flex align-items-center justify-content-center" style="width: 50px; height: 50px; font-size: 10px;">No Image</div>';
                            }
                            ...
                            tableRows += '<td>' + (row.created_at ? row.created_at : 'N/A') +
                                '</td>';
                            tableRows += '<td>' + (row.updated_at ? row.updated_at : 'N/A') +
                                '</td>';
                            tableRows += '<td>' +
                                ...
                                '" class="btn btn-outline-danger btn-sm delete-author-btn" style="padding: 2px 8px; font-size: 0.75rem;"><i class="fa fa-trash"></i> Delete</button>' +
                                '</td>';
                            tableRows += '</tr>';
                        });
                    } else {
                        tableRows =
                            '<tr><td colspan="7" class="text-center text-muted py-4">No authors found inside the system database.</td></tr>';
                    }
